...
export function AnalyticsPage() {
  ...
  const commonOptions = { responsive: true, maintainAspectRatio: false, plugins: { legend: { labels: { color: '#8e9aaa' } } }, scales: { x: { ticks: { color: '#8e9aaa' }, grid: { color: 'rgba(128,128,128,.12)' } }, y: { ticks: { color: '#8e9aaa' }, grid: { color: 'rgba(128,128,128,.12)' } } } }
  ...
  return (
    <>
      ...
      <section className="summary-grid" aria-label="Основные показатели">
        <article><span>Предложений</span><strong>{deals.length}</strong></article>
        <article><span>Средняя скидка</span><strong>{formatPercent(analytics.averageSaving)}</strong></article>
        <article><span>Средняя цена</span><strong>{formatPrice(analytics.averagePrice)}</strong></article>
        <article><span>Максимальная скидка</span><strong>{formatPercent(analytics.maximumSaving)}</strong></article>
      </section>
